...
//==============================================================================
uint8_t nrf24_read(uint8_t Cmd, uint8_t *pBuff, uint8_t Len)
{
  NRF24_CSN_LOW();

  // Передаём байт команды, параллельно принимаем байт состояния
  uint8_t State = SPI_SendRecvByte(Cmd);
  // Принимаем указанное кол-во байт
  SPI_recv8b(pBuff, Len);
...
// Возвращает статусный байт
//==============================================================================
uint8_t nrf24_write(uint8_t Cmd, uint8_t *pBuff, uint8_t Len)
{
  NRF24_CSN_LOW();
...
uint8_t nrf24_read_reg(uint8_t RegAddr)
{
  ...
  uint8_t Reg = 0;
  nrf24_read(NRF24_CMD_R_REGISTER | RegAddr, &Reg, 1);
  return Reg;
}
//==============================================================================
...
//==============================================================================
uint8_t nrf24_read_regs(uint8_t RegAddr, uint8_t *pBuff, uint8_t Len)
{
  RegAddr &= NRF24_REGADDR_MASK;
  return nrf24_read(NRF24_CMD_R_REGISTER | RegAddr, pBuff, Len);
}
//==============================================================================
...
//==============================================================================
void nrf24_write_reg(uint8_t RegAddr, uint8_t Value)
{
  RegAddr &= NRF24_REGADDR_MASK;
  nrf24_write(NRF24_CMD_W_REGISTER | RegAddr, &Value, 1);
}
//==============================================================================
...
//==============================================================================
uint8_t nrf24_write_regs(uint8_t RegAddr, uint8_t *pBuff, uint8_t Len)
{
  RegAddr &= NRF24_REGADDR_MASK;
  return nrf24_write(NRF24_CMD_W_REGISTER | RegAddr, pBuff, Len);
}
//==============================================================================
...
//==============================================================================
uint8_t nrf24_reuse_tx_lastpayload(void)
{
  return nrf24_write(NRF24_CMD_REUSE_TX_PL, 0, 0);
}
//==============================================================================
...
uint8_t nrf24_activate(void)
{
  uint8_t Buff = 0x73;
  return nrf24_write(NRF24_CMD_ACTIVATE, &Buff, 1);
}
#endif
//==============================================================================
...
uint8_t nrf24_write_ackpayload(uint8_t *pBuff, uint8_t Len)
{
  if (Len > 32)
    Len = 32;
  return nrf24_read(NRF24_CMD_W_ACK_PAYLOAD, pBuff, Len);
}
//==============================================================================
...
{
  Channel &= 0x7F;
  return nrf24_write_regs(NRF24_REG_RF_CH, &Channel, 1);
}
//==============================================================================
...
{
  return nrf24_write(NRF24_CMD_FLUSH_RX, 0, 0);
}
//==============================================================================
...
{
  return nrf24_write(NRF24_CMD_FLUSH_TX, 0, 0);
}
//==============================================================================
...
uint8_t nrf24_Read_RxPayload(uint8_t *pBuff, uint8_t Len)
{
  if (Len > 32)
    Len = 32;
  return nrf24_read(NRF24_CMD_R_RX_PAYLOAD, pBuff, Len);
}
//==============================================================================
...
uint8_t nrf24_Write_TxPayload(uint8_t *pBuff, uint8_t Len)
{
  if (Len > 32)
    Len = 32;
  return nrf24_write(NRF24_CMD_W_TX_PAYLOAD, pBuff, Len);
}
//==============================================================================
...
void nrf24_SetOutputPower(uint8_t PowerLevel)
{
  ...
  nrf24_read_regs(NRF24_REG_RF_SETUP, (uint8_t *) &RegValue, 1);
  RegValue.bRF_PWR = (PowerLevel > 3) ? 3 : PowerLevel; 
  nrf24_write_regs(NRF24_REG_RF_SETUP, (uint8_t *) &RegValue, 1);
}
//==============================================================================
...
void nrf24_SetDynLen(uint8_t Pipe, uint8_t Enable)
{
  ...
  nrf24_read_regs(NRF24_REG_DYNPD, &RegValue, 1);
  ...
  nrf24_write_regs(NRF24_REG_DYNPD, &RegValue, 1);
}
//==============================================================================
...
{
  uint8_t Size = 0;
  nrf24_read(NRF24_CMD_R_RX_PL_WID, &Size, 1);
  return Size;
}
//==============================================================================
...
void nrf24_DynPayload_OnOff(uint8_t Enable)
{
  ...
  nrf24_read_regs(NRF24_REG_FEATURE, (uint8_t *) &RegValue, 1);
  RegValue.bEN_DPL = (Enable) ? 1 : 0;
  nrf24_write_regs(NRF24_REG_FEATURE, (uint8_t *) &RegValue, 1);
}
//==============================================================================
...
void nrf24_AckPayload_OnOff(uint8_t Enable)
{
  ...
  nrf24_write_reg(NRF24_REG_DYNPD, (1 << 0) | (1 << 1));
}
//==============================================================================
...
void nrf24_AutoRetrasmission_Setup(uint8_t TryCount, uint8_t TryPeriod)
{
  ...
  nrf24_write_regs(NRF24_REG_SETUP_RETR, (uint8_t *) &RegValue, 1);
}
//==============================================================================
...
{
  return nrf24_read_reg(NRF24_REG_CD) & (1<<0);
}
//==============================================================================
...
void nrf24_setAutoAck(uint8_t Pipe, uint8_t Enable)
{
  ...
  uint8_t RegValue = nrf24_read_reg(NRF24_REG_EN_AA);
  if (Enable)
    RegValue |= (1 << Pipe);
  else
    RegValue &= ~(1 << Pipe);

  nrf24_write_reg(NRF24_REG_EN_AA, RegValue);
}
//==============================================================================
...
void nrf24_SendAckPayload(uint8_t Pipe, uint8_t *pBuff, uint8_t Len)
{
  ...
  nrf24_write(NRF24_CMD_W_ACK_PAYLOAD | Pipe, pBuff, Len);
}
//==============================================================================
...
void nrf24_Set_PRX_Mode(void)
{
  ...
  nrf24_write_regs(NRF24_REG_RX_ADDR_P0, RxPipes[0].Address, 5);
...
void nrf24_RxPipe_Setup(uint8_t Pipe, uint8_t *pAddress, uint8_t PayloadSize)
{
  ...
  if (Pipe < 2)
    nrf24_write_regs(NRF24_REG_RX_ADDR_P0 + Pipe, pAddress, 5);
  else
    nrf24_write_regs(NRF24_REG_RX_ADDR_P0 + Pipe, pAddress, 1);

  if (!RxPipes[Pipe].bDynPayLoad)       // Размер пакета статический
  {
    // Устанавливаем размер пакета для соединения
    nrf24_write_reg(NRF24_REG_RX_PW_P0 + Pipe, PayloadSize);
  }
  
  // Включаем приём из указанного соединения (pipe)
  uint8_t RegValue = nrf24_read_reg(NRF24_REG_EN_RXADDR);
  RegValue |= (1 << Pipe);
  nrf24_write_reg(NRF24_REG_EN_RXADDR, RegValue);
}
//==============================================================================
...
int8_t nrf24_Send(uint8_t *pAddress, uint8_t *pBuff, uint8_t Len)
{
  ...
  nrf24_write_regs(NRF24_REG_RX_ADDR_P0, pAddress, 5);
  ...
  NRF24_CE_HIGH();
  _delay_us(10);         // Задержка минимум 10 мкс
  NRF24_CE_LOW();
  ...
  if (State & NRF24_bTX_DS_Mask)// Пакет был успешно отправлен (ACK получен), возвращаем кол-во выполненных попыток
  {
    // Читаем счётчики отправки
    tNrf24Reg08h Observe;
    nrf24_read_regs(NRF24_REG_OBSERVE_TX, (uint8_t *) &Observe, 1);
...
{
  return nrf24_read(NRF24_CMD_NOP, 0, 0);
}
//==============================================================================
...
void nrf24_init(uint8_t Channel)
{
  ...
  NRF24_CSN_DDR |= NRF24_CSN_Mask;
  NRF24_CE_DDR |= NRF24_CE_Mask;
  NRF24_CSN_HIGH();
  NRF24_CE_HIGH();
  // Инициализируем SPI
  spim_init();
  ...
  nrf24_AutoRetrasmission_Setup(15, 4); // 15 попыток с периодом 1.25 мс
